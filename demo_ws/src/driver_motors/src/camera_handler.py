...
def camera_handler():
	...
	rospy.Subscriber("/controls", PointStamped, callbackControl)
	rospy.Subscriber("/mavros/vision_pose/pose", PoseStamped, callbackPose)

	pubRPYref = rospy.Publisher('/rpy/ref', Vector3Stamped, queue_size=1)
	
	pubRPY = rospy.Publisher('/rpy/camera/kalman', Vector3Stamped, queue_size=1)
	pubRPY2 = rospy.Publisher('/rpy/camera/2degree', Vector3Stamped, queue_size=1)
	pubVel = rospy.Publisher('/vel/camera/kalman', Vector3Stamped, queue_size=1)
	pubVel2 = rospy.Publisher('/vel/camera/2degree', Vector3Stamped, queue_size=1)
	...
	while not rospy.is_shutdown():
		...
		control[0] = glob_control_msg.point.x
		control[1] = glob_control_msg.point.y
		control[2] = glob_control_msg.point.z
		control[3] = 0
		...
		# SECOND DEGREE
		x_cam_measure = glob_pose_msg.pose.position.x
		y_cam_measure = glob_pose_msg.pose.position.y
		z_cam_measure = glob_pose_msg.pose.position.z
		...
		x_cam_dot2 += (-k1_dxy * x_cam_dot2 - k2_dxy * x_cam2 + k_dxy * x_cam_measure) * dt
		x_cam2 += x_cam_dot2 * dt
		y_cam_dot2 += (-k1_dxy * y_cam_dot2 - k2_dxy * y_cam2 + k_dxy * y_cam_measure) * dt
		y_cam2 += y_cam_dot2 * dt
		z_cam_dot2 += (-k1_dxy * z_cam_dot2 - k2_dxy * z_cam2 + k_dxy * z_cam_measure) * dt
		z_cam2 += z_cam_dot2 * dt

		#rospy.loginfo("x_cam=%f x_cam_dot=%f", x_cam, x_cam_dot);
		
		# Publishers
		msg_rpy_cam = Vector3Stamped()
		msg_rpy_cam.header.stamp = rospy.get_rostime()
		msg_rpy_cam.vector.x = x_estimated[0]
		msg_rpy_cam.vector.y = x_estimated[1]
		msg_rpy_cam.vector.z = x_estimated[2]
		
		msg_rpy_cam2 = Vector3Stamped()
		msg_rpy_cam2.header.stamp = rospy.get_rostime()
		msg_rpy_cam2.vector.x = x_cam2
		msg_rpy_cam2.vector.y = y_cam2
		msg_rpy_cam2.vector.z = z_cam2
