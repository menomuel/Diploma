...
{	
	...
	k_teta = a_teta = k_phi = a_phi = k_psi = a_psi = 12; // flight - 10.0, model - 16.0; 
	...
	while (ros::ok())
	{
		...
		if (imuUp && camRefUp)
		{
			...
			*/
			...
			if (accel_norm_sq > lower_accel_limit*lower_accel_limit)
			{
				double bias_phi = -0.05;
				double bias_teta = 0.05; // 0.1; // 0.04
				
				phi_measure = std::atan(acc_y / sqrt(acc_x*acc_x + acc_z*acc_z));
				teta_measure = - std::atan(acc_x / sqrt(acc_y*acc_y + acc_z*acc_z));
				phi_pf = std::atan(acc_filter_val_y / sqrt(acc_filter_val_x*acc_filter_val_x + acc_filter_val_z*acc_filter_val_z)) - bias_phi;
				teta_pf = - std::atan(acc_filter_val_x / sqrt(acc_filter_val_y*acc_filter_val_y + acc_filter_val_z*acc_filter_val_z)) - bias_teta;
			}
			psi_measure = glob_camRPY_msg.vector.z; // from camera
			
			//ROS_INFO("%f %f %f", phi_pf, teta_pf, psi_pf);
			...
			double freql = 0.5; // [Hz]
			double freqa = freql * 2 * pi;
			
			double timelim_idle = 10; // [s]
			double timecurr = ros::Time::now().toSec(); 
			if (timecurr - timestart > timelim_idle)
			{
				teta_ref = -0.2 + 0.2*std::sin(freqa*(timecurr-timelim_idle));
				w_y_ref = 0.2 * freqa * std::cos(freqa*(timecurr-timelim_idle));
			}
			*/
